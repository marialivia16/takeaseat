...
function addressCoordinates(address) {
    geocoder.geocode({ 'address': address }, function (results, status) {
        if (status == google.maps.GeocoderStatus.OK) {
            ...
            $("#hiddenLat").val(results[0].geometry.location.lat());
            $("#hiddenLng").val(results[0].geometry.location.lng());
            return true;
        }
        else {
            return false;
...
$(document).on("click", ".btnMakeReservation", function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            //alert("success");
            $("#reservationConfirmation-" + restId + " p").html(response);
            $("#reservationConfirmation-" + restId + " p").removeClass("hidden").addClass("in");
