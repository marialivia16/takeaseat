/*!
 * $ JavaScript Library v2.1.3
 * http://jquery.com/
 *
 * Includes Sizzle.js
 * http://sizzlejs.com/
 *
 * Copyright 2005, 2014 $ Foundation, Inc. and other contributors
	...
	add: function( elem, types, handler, data, selector ) {
		...
		if ( !(eventHandle = elemData.handle) ) {
			eventHandle = elemData.handle = function( e ) {
				...
				return typeof $ !== strundefined && $.event.triggered !== e.type ?
				...
				if ( hasScripts ) {
					...
					for ( i = 0; i < hasScripts; i++ ) {
						...
						if ( rscriptType.test( node.type || "" ) &&
							!data_priv.access( node, "globalEval" ) && $.contains( doc, node ) ) {
	...
	for ( ; index < length; index++ ) {
		...
		if ( show ) {
			...
			if ( display !== "none" || !hidden ) {
				data_priv.set( elem, "olddisplay", hidden ? display : $.css( elem, "display" ) );
			}
		}
	}
